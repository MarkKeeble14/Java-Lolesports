...
public class Driver {
	...
	public static void SimulateCurrentFormatFromScratch() throws Exception {
		...
		TournamentFormat WC = new TournamentWorldChampionship();
		WC.Simulate(pools);
	}
	...
	public static void SimulateCurrentWorldsState() throws Exception {
		TournamentWorldChampionship.SimulateCurrentWorldsState();
	}
	
	public static void SimulateCurrentMSIFormatFromScratch() throws Exception {
		// Setting up Pools
		Pool P1 = new Pool(Strings.LMSIGroupsPoolOne, Teams.RNG, Teams.DK, Teams.PSG, Teams.C9, Teams.MAD, Teams.GAM); 
		Pool P2 = new Pool(Strings.LMSIGroupsPoolTwo, Teams.PGG, Teams.UOL, Teams.PNG, Teams.IW, Teams.DFM, Teams.INF);
		List<Pool> pools = new ArrayList<Pool>(Arrays.asList(P1, P2));
		
		TournamentFormat MSI = new TournamentMSI();
		MSI.Simulate(pools);
	}
}
